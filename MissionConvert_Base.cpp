...
//Init archetype obtainer. Shared across all instances of mission convert.
EntArchetypeObtainer MissionConvert_Base::entarc = EntArchetypeObtainer();

void MissionConvert_Base::VersionSpecificInstructions()
{
}

bool MissionConvert_Base::Convert()
{
...
bool MissionConvert_Base::SaveToDisk()
{
	...
	for (auto& x : m_layer_names)
	{
		...
		//We need to save .lyr files in folder of level in case of converting multiple levels in a row.
		if (dc.save_file(std::string(Util::PathWithoutFilename(xml_path) + x + ".lyr").c_str(), " ") == false) //unsuccessful
		{
			successful = false;
		}
	}
...
void MissionConvert_Base::ExtractTOD()
{
	...
	{
		g_Log.Log("Can't write TOD file to disk!", this, ERROR);
	}
	else
	{
